...
def calculate_proximity_score(
    ...
    # GROUND PENALTY: Ground vehicles and grounded aircraft get massive penalty
    if on_ground or altitude_ft < 100:
        return base_score * 0.01  # 99% penalty - essentially filter them out

    # Approach bonus: More lenient for high-altitude flights
    if altitude_ft > 25000:
        # High altitude flights (cruising) - heading matters less, just show them
        approach_multiplier = 1.2 if is_approaching else 1.0
    else:
        # Low altitude - approach direction matters more
        approach_multiplier = 1.5 if is_approaching else 0.7

    # Altitude factor: AIRBORNE = PRIORITY
    # All airborne aircraft are relevant, prioritize based on phase of flight
    altitude_multiplier = 1.0
    if altitude_ft < 5000:
        # Landing/taking off = very relevant and close
        altitude_multiplier = 1.4
    elif altitude_ft < 15000:
        # Low cruising/climbing/descending = relevant
        altitude_multiplier = 1.2
    elif altitude_ft > 30000:
        # High cruising = still important (overhead flights like LHR-JFK)
        altitude_multiplier = 1.1  # CHANGED from 0.8 penalty to 1.1 bonus!
...
def calculate_aircraft_proximity(
    ...
    aircraft_heading: int,
    aircraft_on_ground: bool = False
) -> Tuple[float, float, float, bool, float, float]:
    """
    Calculate all proximity metrics for an aircraft

    Returns:
        Tuple of (horizontal_distance_mi, distance_3d_mi, bearing, is_approaching, proximity_score, heading_difference)
    """
    ...
    # Calculate target heading for approaching (reverse bearing)
    target_heading = (bearing + 180) % 360
    ...
    approaching = is_aircraft_approaching(bearing, aircraft_heading, threshold_degrees=45)

    # Calculate proximity score
    score = calculate_proximity_score(distance_3d, approaching, aircraft_altitude_ft, aircraft_on_ground)

    return (horizontal_distance, distance_3d, bearing, approaching, score, heading_diff)
